...
video = cv2.VideoWriter('result.avi', cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'), fps, (height2, width2), 1)
...
while True:
    ...
    if ret:  # if there is a frame continue with code
        ...
        bins = cv2.morphologyEx(fgmask, cv2.MORPH_CLOSE, kernel)
        ...
        lineypos = 225
        cv2.line(image, (0, lineypos), (width, lineypos), (0, 165, 255), 5)

        # line y position created to count contours
        lineypos2 = 250
        cv2.line(image, (0, lineypos2), (width, lineypos2), (0,100,255), 5)
        ...
        cv2.putText(image, "ROI_Object: " + str(currentcars), (0, 15), cv2.FONT_HERSHEY_SIMPLEX, .5, (255, 0, 0), 2)

        cv2.putText(image, "ROI_Up: " + str(carscrossedup), (0, 30), cv2.FONT_HERSHEY_SIMPLEX, .5, (255, 0, 0),
                    2)

        cv2.putText(image, "ROI_Down: " + str(carscrosseddown), (0, 45), cv2.FONT_HERSHEY_SIMPLEX, .5,
                    (255, 0, 0), 2)

        cv2.putText(image, "TOTAL COUNT: " + str(len(carids)), (0, 60), cv2.FONT_HERSHEY_SIMPLEX, .5,
                    (255, 0, 0), 2)

        cv2.putText(image, "FRAME: " + str(framenumber) + ' of ' + str(frames_count), (0, 75), cv2.FONT_HERSHEY_SIMPLEX,
                    .5, (255, 0, 0), 2)

        cv2.putText(image, 'TIME: ' + str(round(framenumber / fps, 2)) + ' s / ' + str(round(frames_count / fps, 2))
                    + ' s', (0, 90), cv2.FONT_HERSHEY_SIMPLEX, .5, (255, 0, 0), 2)

        # displays images and transformations
        cv2.imshow("LAST_Processing", image)
        # cv2.imshow("COUNTOURS", image)
        # cv2.moveWindow("countours", 0, 0)

        cv2.imshow("CV_INSIDE", fgmask)
...
df.to_csv('vehicle_data.csv', sep=',')
